...
<html lang="en">
  ...
  <head>
    ...
    <script>
      ...
      {
        window.open("http://agilepartner.comxa.com/index.php/editDefect","_blank","toolbar=yes, scrollbars=yes, resizable=yes, top=10, left=175, width=1000, height=640");
      }
...
<?php
  ...
  function loadDefects(){
    ...
    $query = "SELECT DISTINCT d.id,d.name,d.planEstimate,d.priority,d.owner,d.state,u.fName,u.lName 
              FROM defect d,users u WHERE d.owner=u.username AND project='$project' ORDER BY d.id ASC";
    $result = mysql_query($query);

    echo "<center><div class='datagrid' style='width:1320px'>
          <table id='defectsTable' class='table_US' >
             <thead><tr>
              <th width='5%'><img src='".base_url('assets\images\defect_icon.png')."'></img>&nbsp;ID</th>
              <th width='35%'>NAME</th>
    ...
    while($row = mysql_fetch_array($result)){   //Creates a loop to loop through results
      ...
      echo "<tbody><tr class='alt'><td>" . $row['id'] . "</td><td>" . $row['name'] . "</td><td>" . $row['state'] . "</td>
    ...
    /* if($_SESSION['teamRole']=='Scrum Master'||$_SESSION['permission']=='delete'||$_SESSION['permission']=='edit/delete'&& $deleteStatus){
    ...
    $query = "SELECT DISTINCT d.id,d.name,d.planEstimate,d.priority,d.owner,d.submittedBy,d.state FROM defect d,users u WHERE d.owner='noEntry' AND project='$project' ORDER BY d.id ASC";
    $result = mysql_query($query);

     while($row = mysql_fetch_array($result)){   //Creates a loop to loop through results

      echo "<tbody><tr class='alt'><td>" . $row['id'] . "</td><td>" . $row['name'] . "</td><td>" . $row['state'] . "</td>
            <td>" . $row['priority'] . "</td><td>".'&nbsp;'."</td><td><a href='http://agilepartner.comxa.com/index.php/viewDefect/get_values/". $row['id'] ."'> <img src='".base_url('assets\images\view_icon.png')."' > 
            </img> </a></td>";
