<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class ReportsDefectsScheduleState extends CI_Controller{
		...
		public function load($project,$sortBy){
			...
			//Defects
			if($project=="All"){
				$query = $this->db->query("SELECT DISTINCT d.id,d.name,d.description,d.scheduleState,d.state,d.iteration,d.planEstimate,d.priority,d.severity,d.creationDate,d.targetDate,d.owner,u.fName,u.lName FROM defect d,users u WHERE d.owner=u.username ORDER BY d.$sortBy ASC");
			}
			elseif($project!="All"){
				$query = $this->db->query("SELECT DISTINCT d.id,d.name,d.description,d.scheduleState,d.state,d.iteration,d.planEstimate,d.priority,d.severity,d.creationDate,d.targetDate,d.owner,u.fName,u.lName FROM defect d,users u WHERE d.owner=u.username AND d.project='$project' ORDER BY d.$sortBy ASC");
			}
			...
			//graph
			$data['graph'] = $this->model_reports_defects->countDefectsByScheduleState($project);

			$this->load->view('view_header');
			$this->load->view('view_reports_defects_scheduleState',$data);
		}
