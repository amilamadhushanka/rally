	...
	class EditUserstory extends CI_Controller{
		...
		public function update(){
			...
			if($this->form_validation->run()==FALSE){
				...
				$this->load->view('view_editUserstory');
			}
			else{
				//Successful validation

				$this->load->model('model_edit_userstory');

				$result=$this->model_edit_userstory->update_userstory();

				if($result){
					echo "<script>alert('Userstory updated successfully!')</script>";
					redirect('http://agilepartner.comxa.com/index.php/backlog/view_userStories', 'refresh'); 
				}
				else{
		...
		public function get_values($id){
			...
			$this->load->view('view_editUserstory',$data);
		}
	}
