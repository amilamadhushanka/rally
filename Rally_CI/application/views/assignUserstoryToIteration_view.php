  ...
  function loadUserstory($project){
    ...
    while($row = mysql_fetch_array($result)){   //Creates a loop to loop through results
      ...
      echo "<tbody><tr class='alt'><td>" . $row['ID'] . "</td><td>" . $row['Name'] . "</td><td>" . $row['Discription'] . "</td><td>" . $row['Plan_estima'] . "</td>
            <td>" . $row['Priority'] . "</td><td><a href='http://localhost/Rally_CI/UsAssignedToIteration/get_values/". $row['ID'] ."/". $row['pname'] ."'> <img src='".base_url('assets\images\assign_icon.png')."'></img> </a></td></tr></tbody>";
    }
