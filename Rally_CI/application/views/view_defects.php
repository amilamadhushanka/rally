...
<html lang="en">
  ...
  <head>
    ...
    <script>
      ...
      {
        window.open("http://localhost/Rally_CI/editDefect","_blank","toolbar=yes, scrollbars=yes, resizable=yes, top=10, left=175, width=1000, height=640");
      }
      ...
      <td style="text-align:right;">Show:&nbsp;
        <td>
          <select id="view" name="view" onChange="changeFunc();">
            ...
            <option value="df" selected>Defects</option>
...
<?php
  ...
  function loadDefects(){
    ...
    $query = "SELECT DISTINCT d.id,d.name,d.planEstimate,d.priority,d.owner,u.fName,u.lName FROM defect d,users u WHERE d.owner=u.username ORDER BY d.id ASC";
    ...
    while($row = mysql_fetch_array($result)){   //Creates a loop to loop through results
      ...
      echo "<tbody><tr class='alt'><td>" . $row['id'] . "</td><td>" . $row['name'] . "</td><td>" . $row['planEstimate'] . "</td>
            <td>" . $row['priority'] . "</td><td>" . $row['fName'] . '&nbsp;' . $row['lName'] . "</td><td><a href='http://localhost/Rally_CI/editDefect/get_values/". $row['id'] ."'> <img src='".base_url('assets\images\edit_icon.png')."' > 
            </img> </a></td><td><a href='http://localhost/Rally_CI/deleteDefect/get_values/". $row['id'] ."'> <img src='".base_url('assets\images\delete_icon.png')."' > 
            </img> </a></td></tr></tbody>";
    }

    $query = "SELECT DISTINCT d.id,d.name,d.planEstimate,d.priority,d.owner FROM defect d,users u WHERE d.owner='noEntry' ORDER BY d.id ASC";
    $result = mysql_query($query);

     while($row = mysql_fetch_array($result)){   //Creates a loop to loop through results
      
      echo "<tbody><tr class='alt'><td>" . $row['id'] . "</td><td>" . $row['name'] . "</td><td>" . $row['planEstimate'] . "</td>
            <td>" . $row['priority'] . "</td><td>".'&nbsp;'."</td><td><a href='http://localhost/Rally_CI/editDefect/get_values/". $row['id'] ."'> <img src='".base_url('assets\images\edit_icon.png')."' > 
            </img> </a></td><td><a href='http://localhost/Rally_CI/deleteDefect/get_values/". $row['id'] ."'> <img src='".base_url('assets\images\delete_icon.png')."' > 
            </img> </a></td></tr></tbody>";
    }
