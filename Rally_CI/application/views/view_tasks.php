...
<html lang="en">
  ...
  <head>
    ...
    <script>
      function openWin()
      {
        window.open("http://localhost/Rally_CI/viewTask_selectProject","_blank","toolbar=yes, scrollbars=yes, resizable=yes, top=10, left=175, width=1000, height=640");
      }

      function openWin1()
      {
        window.open("http://localhost/Rally_CI/viewTask_selectUser","_blank","toolbar=yes, scrollbars=yes, resizable=yes, top=10, left=175, width=1000, height=640");
      }
  ...
  <table>
    ...
    <tr>
      ...
      <td>
          <form>
            <button onclick="openWin1()" class="btn btn-default btn-lg">View Assign Tasks</button>
          </form> 
        </td>
        <td >&nbsp;</td>
        <td>
          <form>
            <button onclick="openWin()" class="btn btn-default btn-lg">Assign Tasks</button>
          </form>
        </td>
        <td >&nbsp;</td>
        <td>
...
<?php
  ...
  function loadTasks(){
    ...
    $query = "SELECT * FROM newtask ORDER BY task_name ASC";
    ...
    while($row = mysql_fetch_array($result)){   //Creates a loop to loop through results
      echo "<tbody><tr class='alt'><td>" . $row['task_name'] . "</td><td><a href='http://localhost/Rally_CI/editTask/get_values/". $row['task_name'] ."'>
             <img src='".base_url('assets\images\edit_icon.png')."' > </img> </a></td><td><a href='http://localhost/Rally_CI/deleteTask/get_values/". $row['task_name'] ."'>
